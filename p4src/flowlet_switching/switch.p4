...
@name(".flowlet_lasttime") register<bit<48>>(32w8192) flowlet_lasttime;
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".lookup_flowlet_map") action lookup_flowlet_map() {
        hash(meta.ingress_metadata.flowlet_map_index, HashAlgorithm.crc16, (bit<13>)0, { hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort }, (bit<26>)8192);
        ...
        flowlet_lasttime.write((bit<32>)meta.ingress_metadata.flowlet_map_index, (bit<48>)meta.intrinsic_metadata.ingress_global_timestamp);
    }
    ...
    apply {
        flowlet.apply();
        if (meta.ingress_metadata.flow_ipg > 48w50000) {
            new_flowlet.apply();
        }
        ecmp_group.apply();
        ecmp_nhop.apply();
