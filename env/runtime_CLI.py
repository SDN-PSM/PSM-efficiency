...
class RuntimeAPI(cmd.Cmd):
    ...
    @handle_bad_input
    def do_counter_read_all(self, line):
        "Read counter value: counter_read_all <name>"
        args = line.split()
        self.exactly_n_args(args, 1)
        counter_name = args[0]
        counter = self.get_res("counter", counter_name, ResType.counter_array)
        if counter.is_direct:
            ...
            print "this is the direct counter for table", table_name
            # index = index & 0xffffffff
            for idx in range(0, counter.size):
                value = self.client.bm_mt_read_counter(0, table_name, idx)
                print "%s[%d]= " % (counter_name, idx), value
        else:
            for idx in range(0, counter.size):
                value = self.client.bm_counter_read(0, counter.name, idx)
                print "%s[%d]= " % (counter_name, idx), value
